...
export default function InviteTeacher({open, setOpen , classCode}) {
  ...
  const handleInviteTeacherByEmail = async () => {
    setLoading(true);
    if (email === '') {
      ...
      await toast.promise(ClassService.inviteByEmail(
        {email : email, classCode : classCode, isTeacher : true})
        .then(() => {
  ...
  return (
    <>
      <Dialog open={open} onClose={handleClose}>
        ...
        <DialogContent>
          <DialogContentText>
            To invite teacher to this class, please provide the teacher's email address here.
            We will send the invitation link to this teacher's email.
          </DialogContentText>
          <TextField
            autoFocus
            margin="dense"
        ...
        <DialogActions>
          ...
          <LoadingButton
            onClick={handleInviteTeacherByEmail}
            variant="contained"
